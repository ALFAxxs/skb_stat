...
def export_file_download(request, filename):
    ...
    elif filename.startswith('opservices'):
        display = 'operatsiya_xizmatlari.xlsx'
    elif filename.startswith('operations'):
        display = 'operatsiya_statistika.xlsx'
    else:
...
def export_operation_excel(request):
    ...
    params = request.GET.urlencode()
    messages.info(request, _("Excel yuklab olish uchun '📥 Excel' tugmasini bosing."))
    return redirect(f'/services/operations/statistics/?{params}' if params else '/services/operations/statistics/')


@login_required
def export_operations_start(request):
    """Operatsiyalar Excel task'ini Celery'ga yuboradi → {task_id}"""
    from .export_tasks import generate_operations_excel
    filters = {k: v for k, v in {
        'date_from':  request.GET.get('date_from'),
        'date_to':    request.GET.get('date_to'),
    ...
    task = generate_operations_excel.delay(filters)
    return JsonResponse({'task_id': task.id})
...
def export_operation_service_excel(request):
    ...
    params = request.GET.urlencode()
    messages.info(request, _("Excel yuklab olish uchun '📥 Excel' tugmasini bosing."))
    return redirect(f'/services/operations/service-statistics/?{params}' if params else '/services/operations/service-statistics/')


@login_required
def export_operation_services_start(request):
    """Operatsiya xizmatlari Excel task'ini Celery'ga yuboradi → {task_id}"""
    from .export_tasks import generate_operation_services_excel
    filters = {k: v for k, v in {
        'date_from':        request.GET.get('date_from'),
        'date_to':          request.GET.get('date_to'),
        'category':         request.GET.get('category'),
    ...
    task = generate_operation_services_excel.delay(filters)
    return JsonResponse({'task_id': task.id})
